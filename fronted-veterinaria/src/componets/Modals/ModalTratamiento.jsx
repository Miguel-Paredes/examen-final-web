...
const ModalTratamiento = ({ idPaciente }) => {
  const { setModal, handleModal, registrarTratamientos } = useContext(
    TratamientosContext
  );
    ...
    <div className="lg:w-2/4 lg:h-3/5 bg-gray-800 bg-opacity-100 top-1/4 left-1/3 fixed sticky-0 rounded-lg overflow-y-scroll">
      ...
      <form className="p-10" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          {errors.prioridad && (
            <p className="text-red-500">Prioridad es requerida</p>
          )}
        </div>

        <div>
          <label className="text-white uppercase font-bold text-sm">
            ID Paciente:
          </label>
          <input
            type="text"
